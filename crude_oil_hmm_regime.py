"""
Crude Oil HMM Regime Detection

Uses a Gaussian Hidden Markov Model to identify market regimes (e.g. bull, bear,
high/low volatility) from crude oil price and volume. Same algorithm as the generic
engine: features are Returns, Range, Vol_Change from OHLCV. Built for crude oil via
yfinance (default: CL=F for WTI crude oil futures).
"""
...
def fit_and_predict(
    ...
    """
    Download crude oil data, engineer features, fit Gaussian HMM (or GMM fallback), and return regime summary + states.
...
def plot_regimes(
    ...
    ax.set_title(
        title or f"Crude Oil ({result.symbol}) Close — Market Regimes (Last {n} bars)",
        fontsize=14,
    )
    ax.set_xlabel("Bar index")
    ax.set_ylabel(f"Crude Oil ({result.symbol}) Price")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Hidden Markov Model regime detection for crude oil (yfinance)."
    )
    parser.add_argument(
        "symbol",
        nargs="?",
        default="CL=F",
        help="Crude oil symbol (default CL=F for WTI futures)",
    )
    parser.add_argument(
        "--period",
        default="730d",
    ...
    print("\n=== Crude Oil Market Regime Summary ===")
    print(
        pd.DataFrame(
            [
                {
