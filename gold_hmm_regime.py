"""
Gold HMM Regime Detection

Uses a Gaussian Hidden Markov Model to identify market regimes (e.g. bull, bear,
high/low volatility) from gold price and volume. Same algorithm as the generic
engine: features are Returns, Range, Vol_Change from OHLCV. Built for gold via
yfinance (default: GC=F for Comex gold futures; use GLD for SPDR Gold ETF).
"""
...
def fit_and_predict(
    ...
    """
    Download gold data, engineer features, fit Gaussian HMM (or GMM fallback), and return regime summary + states.
...
def plot_regimes(
    ...
    ax.set_title(
        title or f"Gold ({result.symbol}) Close — Market Regimes (Last {n} bars)",
        fontsize=14,
    )
    ax.set_xlabel("Bar index")
    ax.set_ylabel(f"Gold ({result.symbol}) Price")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Hidden Markov Model regime detection for gold (yfinance)."
    )
    parser.add_argument(
        "symbol",
        nargs="?",
        default="GC=F",
        help="Gold symbol (default GC=F; use GLD for ETF)",
    )
    parser.add_argument(
        "--period",
        default="730d",
