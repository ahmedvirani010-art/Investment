...
export function FundamentalResultCard({ ticker }: { ticker: string }) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<FundamentalResult | null>(null);
  const [deepLoading, setDeepLoading] = useState(false);

  const fetchFundamental = useCallback((mode: "quick" | "deep") => {
    if (mode === "deep") setDeepLoading(true);
    const params = new URLSearchParams({ mode, symbol: ticker });
  ...
  useEffect(() => {
    ...
    setError(null);
    setResult(null);
    fetchFundamental("quick");
  }, [ticker, fetchFundamental]);
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-lg border p-3">
              ...
              <p className="text-xl font-semibold tabular-nums">{result.valuation_score.toFixed(0)}</p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-xs text-muted-foreground mb-1">Health</p>
              <p className="text-xl font-semibold tabular-nums">{result.health_score.toFixed(0)}</p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-xs text-muted-foreground mb-1">Growth</p>
              <p className="text-xl font-semibold tabular-nums">{result.growth_score.toFixed(0)}</p>
            </div>
            <div className="rounded-lg border p-3">
              <p className="text-xs text-muted-foreground mb-1">Momentum</p>
              <p className="text-xl font-semibold tabular-nums">{result.momentum_score.toFixed(0)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            <p>P/E: {fmtNum(result.valuation.pe_ratio)} · P/B: {fmtNum(result.valuation.pb_ratio)}</p>
            <p>Div yield: {fmtPct(result.valuation.dividend_yield)} · EV/EBITDA: {fmtNum(result.valuation.ev_ebitda)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-2 text-sm">
            <p>D/E: {fmtNum(result.financial_health.debt_to_equity)} · Current ratio: {fmtNum(result.financial_health.current_ratio)}</p>
            <p>ROA: {fmtPct(result.financial_health.roa)} · ROE: {fmtPct(result.financial_health.roe)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          <p>Revenue growth YoY: {fmtPct(result.growth_metrics.revenue_growth_yoy)} · Earnings growth YoY: {fmtPct(result.growth_metrics.earnings_growth_yoy)}</p>
            ...
            <ul className="space-y-2">
              {result.red_flags.map((r, i) => (
                <li key={i} className="flex items-start gap-2 text-sm">
                  <Badge className={severityStyle[r.severity] ?? "bg-muted text-muted-foreground"}>{r.severity}</Badge>
      ...
      {(result.fair_value != null || result.upside_pct != null) && (
        <Card>
          ...
          <CardContent className="flex flex-wrap gap-6 text-sm">
            {result.fair_value != null && <p>Fair value: {fmtNum(result.fair_value)}</p>}
            {result.upside_pct != null && <p>Upside: {fmtPct(result.upside_pct)}</p>}
          </CardContent>
        </Card>
      )}
