...
export async function GET(request: NextRequest) {
  ...
  return new Promise<NextResponse>((resolve) => {
    ...
    proc.on("close", (code) => {
      if (code !== 0) {
        console.error("NIKKEI HMM regime script stderr:", stderr);
        resolve(
          NextResponse.json(
            {
              success: false,
              error: "NIKKEI HMM regime analysis failed",
      ...
      try {
        ...
        console.error("NIKKEI HMM regime parse error:", stdout?.slice(0, 200));
        resolve(
          NextResponse.json(
            {
              success: false,
              error: "Invalid response from NIKKEI HMM regime script",
    ...
    proc.on("error", (err) => {
      resolve(
        NextResponse.json(
          {
            success: false,
            error: "Failed to run NIKKEI HMM regime analysis",
            details: err.message,
          },
          { status: 500 }
        )
