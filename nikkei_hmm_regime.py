"""
NIKKEI HMM Regime Detection

Uses a Gaussian Hidden Markov Model to identify market regimes (e.g. bull, bear,
high/low volatility) from price and volume. Built for Nikkei 225 (^N225) and
other Japan index yfinance symbols. Same algorithm as the generic engine:
features are Returns, Range, Vol_Change from OHLCV.
"""
...
def fit_and_predict(
    ...
    """
    Download NIKKEI data, engineer features, fit Gaussian HMM (or GMM fallback), and return regime summary + states.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Hidden Markov Model market regime detection for NIKKEI (yfinance: ^N225 for Nikkei 225)."
    )
    parser.add_argument(
        "symbol",
        nargs="?",
        default="^N225",
        help="NIKKEI symbol to analyze (default ^N225 for Nikkei 225)",
    )
    parser.add_argument(
        "--period",
        default="730d",
